...
#     ax.plot(np.array(obs_positions)[:, 1], np.array(obs_positions)[:, 0], 'bo', label='Obstacles')
#     ax.set_title(title)
#     ax.legend()
#     ax.axis('off')
#
...
#     plt.plot(start_node[1], start_node[0], 'bs', markersize=6, label='Start')
...
plan_data = run_planning_simulation(obs_positions, obs_velocities, agent_pos_init, goal_data,
                                    modelPNO, sdf_pred, mask_tensor, device,
...
obs_dots_bin, = ax_bin.plot([], [], 'bo', markersize=8)
...
obs_dots_risk, = ax_risk.plot([], [], 'bo', markersize=8)
